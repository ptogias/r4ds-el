...
Θα βασιστούμε σε πολλά από αυτά που έχετε μάθει στο @sec-data-import, θα συζητήσουμε όμως ακόμα πρόσθετες οπτικές και δυσκολίες κατά την εργασία με δεδομένα από υπολογιστικά φύλλα.
...
Οι περισσότερες από τις λειτουργίες της readxl σας επιτρέπουν να φορτώνετε υπολογιστικά φύλλα Excel στην R:
...
Μπορείτε να ρίξετε μια ματιά ανοίγοντας το αρχείο στο Excel.
...
Και καθώς μετακινείστε προς τα κάτω σε μια στήλη, ο αριθμός στην ετικέτα του κελιού αυξάνεται, για παράδειγμα `A2`, `A3` κ.λπ.
...
Δεν θα μπούμε σε λεπτομέρειες σχετικά με τη χρήση αυτού του πακέτου, αλλά συνιστούμε να διαβάσετε το <https://ycphs.github.io/openxlsx/articles/Formatting.html> για μια εκτενή συζήτηση σχετικά με την περαιτέρω λειτουργικότητα μορφοποίησης για δεδομένα που έχουν γραφτεί από την R σε Excel με την openxlsx.
...
Καθώς η εκμάθηση και η χρήση της R επεκτείνονται εκτός αυτού του βιβλίου, θα συναντήσετε πολλούς διαφορετικούς τρόπους γραφής που χρησιμοποιούνται σε διάφορα πακέτα της R που μπορεί να χρησιμοποιήσετε για να επιτύχετε συγκεκριμένους στόχους.
...
1.  Διαβάστε το σύνολο δεδομένων `students` από νωρίτερα στο κεφάλαιο με το Excel όπως και από το Google Sheets, χωρίς να πρόσθετα ορίσματα στις συναρτήσεις `read_excel()` και `read_sheet()`.
    Είναι ακριβώς τα ίδια τα πλαίσια δεδομένων που προκύπτουν στην R; Αν όχι, σε τι διαφέρουν;
...
Από την άλλη, η εγγραφή σε ένα αρχείο Excel απαιτεί διαφορετικό πακέτο και συνάρτηση (`writexl::write_xlsx()`), ενώ μπορείτε να γράψετε σε ένα Google Sheet με το πακέτο googlesheets4, χρησιμοποιόντας την `write_sheet()`.

Στο επόμενο κεφάλαιο, θα μάθετε για μια διαφορετική πηγή δεδομένων και πως να διαβάζετε δεδομένα από αυτήν στην R: τις βάσεις δεδομένων.
